...
#
# ✅ Local DB priority (no more stale tmp cache)
...
# ✅ New Quote tab: Download button bottom-left; Browse/Edit tab same
# ✅ Word doc: clean Company, vendor, Ship/Bill addresses
# ✅ Bill To: from addresses (Billing + contact/email/phone)
# ✅ Ship To: from addresses (Location/Company) + user contact
# ✅ RE-STOCK: requestor picker before Generate
# ✅ PRAGMA hardening for SQLite reliability
# ✅ Unique widget keys
# ✅ Sidebar shows DB path, file mtime, and newest in-DB date
# --------------------------------------------------------------

from __future__ import annotations
import os, io, re, json, sqlite3, textwrap, hashlib, tempfile
...
APP_VERSION = "2025.10.28-db-local-pref+freshness-bar"
...
def load_config() -> dict:
    # Secrets -> YAML file -> fallback template
    ...
    return {
        "settings": {
            "db_path": str((HERE/DEFAULT_DB).resolve()),
...
def _path_exists_str(p: str|None) -> str|None:
    if not p: return None
    pth = Path(p).expanduser()
    return str(pth.resolve()) if pth.exists() else None
...
def download_db_from_github(*, repo: str, path: str, branch: str='main', token: str|None=None) -> str:
    ...
    out.write_bytes(r.content)  # overwrite each time to avoid stale cache
    return str(out.resolve())

def resolve_db_path(cfg: dict) -> str:
    """
    Strongly prefer LOCAL DB over any downloaded temp cache.
    Priority:
      1) settings.db_path (if exists)
      2) SPF_DB_PATH env var (if exists)
      3) HERE/maintainx_po.db (if exists)
      4) GitHub (only if configured)
      5) HERE/maintainx_po.db (may not exist yet)
    """
    # 1) YAML explicit
    yaml_db = (cfg or {}).get('settings', {}).get('db_path')
    p = _path_exists_str(yaml_db)
    if p: return p

    # 2) ENV
    env_db = os.environ.get('SPF_DB_PATH')
    p = _path_exists_str(env_db)
    if p: return p

    # 3) Local next to app
    local = HERE / DEFAULT_DB
    if local.exists():
        return str(local.resolve())

    # 4) GitHub fallback (only if provided)
...
def newest_in_db_date(db_path: str) -> str:
    """Try to find the newest date across common date columns in both main tables."""
    candidates = {
        "restock": DATE_COLS["restock"],
        "po_outstanding": DATE_COLS["po_outstanding"],
    }
    latest = None
    with open_conn(db_path) as conn:
    ...
    if latest is None:
        return "n/a"
    # Show as date if time not important
    return str(pd.to_datetime(latest).date())


# ============================ APP ============================
cfg = load_config(); cfg = to_plain(cfg)

# IMPORTANT: deep-copy credentials so stauth can mutate safely (avoid SecretsProxy TypeError)
creds_safe = json.loads(json.dumps(cfg.get('credentials', {})))
...
if auth_status is False:
    ...
    # Resolve DB paths (LOCAL takes priority)
    ...
    # Sidebar info + freshness
    db_file = Path(DATA_DB_PATH)
    try:
        ts = db_file.stat().st_mtime
        dt_mtime = datetime.fromtimestamp(ts, tz=timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    except Exception:
        dt_mtime = "n/a"
    newest_date = newest_in_db_date(DATA_DB_PATH)
    ...
    st.sidebar.caption(f"Data DB (source): `{db_file.resolve()}`")
    st.sidebar.caption(f"DB file modified: **{dt_mtime}**")
    st.sidebar.caption(f"Newest date in DB: **{newest_date}**")
    st.sidebar.caption(f"Quotes DB: `{Path(QUOTES_DB_PATH).resolve()}`")
    st.sidebar.caption(f"Quotes in DB: **{quotes_count(QUOTES_DB_PATH)}**")
    if st.sidebar.button("🔄 Refresh data / clear cache", key="btn_refresh_data"):
        st.cache_data.clear()
        st.rerun()
