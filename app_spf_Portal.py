...
# --- ONLY-IN-DOC company cleaner ---
def clean_company_for_doc(s: str) -> str:
    """Strip a leading '<digits> - ' prefix from a company label for DOCX display only."""
...
def quote_docx_bytes(lines: pd.DataFrame, *, vendor: Optional[str], title_companies: str, dataset_label: str) -> bytes:
    """Build the simple quote doc; ONLY here we strip any '<digits> - ' prefix from company."""
    ...
    # Clean company ONLY for the document display
    company_for_doc = clean_company_for_doc(title_companies)
    ...
    doc.add_paragraph(f"{dataset_label} — {company_for_doc}")
        ...
        if st.session_state[cart_key].empty:
            ...
            def build_single_doc() -> bytes:
                return quote_docx_bytes(
                    ...
                    vendor=v_header,
                    title_companies=title_companies,   # RAW is passed; function cleans for DOC only
                    dataset_label=ds
                )
